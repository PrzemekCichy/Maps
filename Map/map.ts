...
declare var object_base,
  ground_base,
  npc_base,
  computed_mob_locations,
  IMAGE_BASE,
  BODY_PARTS,
  NO_HEAD_HELMETS,
  GENDER,
...
class D3Helper {
  ...
  public svgGroups = {
    ...
    monsterTilesHighlight: this.svg
      .append("g")
      .attr("class", "monsterTilesHighlight"),
    clickableTilesHighlight: this.svg
      .append("g")
      .attr("class", "clickableTilesHighlight"),
...
class RpgMap {
  ...
  public RenderNavigation() {
    new Vue({
      ...
      methods: {
        onChange: () => {
          var selectedIndex = map.mapNames.indexOf(
            (<any>document.getElementById("v_dropdown")).value,
          );
      ...
      methods: {
        test: function (property, index) {
          ...
          if (property == "drawEnabledValue") {
            ...
            var list1 = d3
              .selectAll("g")
              .filter("." + renderOptions[index].groupId)
            ...
            var list1 = d3
              .selectAll("g")
              .filter("." + renderOptions[index].groupId)
              .selectAll("polygon,line")
              .attr(
                "stroke-width",
                renderOptions[index].strokeSlider.value.toString(),
              );
            ...
            var list1 = d3
              ...
              .filter("." + renderOptions[index].groupId)
              .selectAll("polygon,line")
              .attr("fill-opacity", renderOptions[index].fillSlider.value);
            ...
            var list1 = d3
              ...
              .filter("." + renderOptions[index].groupId)
              .selectAll("polygon,line")
              .attr("stroke", renderOptions[index].strokeColour);
            ...
            console.log(
              ...
              index,
              renderOptions[index],
              renderOptions[index].fillColour,
            );
            ...
            var list1 = d3
              ...
              .filter("." + renderOptions[index].groupId)
              .selectAll("polygon,line")
              .attr("fill", renderOptions[index].fillColour);
          } else {
          }
  ...
  //Render Map tiles
  public groundTilesCanvas: HTMLCanvasElement = <HTMLCanvasElement>(
    document.getElementById("groundTilesCanvas")
  );
  public ctxGround: CanvasRenderingContext2D =
    this.groundTilesCanvas.getContext("2d");

  public topTilesCanvas: HTMLCanvasElement = <HTMLCanvasElement>(
    document.getElementById("topTilesCanvas")
  );
  public ctxTop: CanvasRenderingContext2D =
    this.topTilesCanvas.getContext("2d");
  ...
  public render(map_id, preserveSvg?) {
    ...
    this.ctxGround.clearRect(
      0,
      0,
      this.groundTilesCanvas.width,
      this.groundTilesCanvas.height,
    );
    this.ctxTop.clearRect(
      0,
      0,
      this.topTilesCanvas.width,
      this.topTilesCanvas.height,
    );
    var offsetX = 0,
      offsetY = 0,
      tile;
    ...
    for (var xx = 100; xx <= 10000; xx += 100) {
      for (var yx = 1; yx <= 100; yx++) {
        ...
        if (typeof ground_base[maps[tile].b_i].top == "object") {
          ...
          this.ctxGround.drawImage(
            IMAGE_BASE[tempImg.sheet].img,
            tempImg.x * IMAGE_BASE[tempImg.sheet].tile_width,
            tempImg.y * 49,
            54,
            49,
            offsetX,
    ...
    for (var x = 0; x < 100; x++) {
      for (var y = 99; y >= 0; y--) {
        ...
        if (on_tile.b_t == "4") {
          if (obj.type == "4") {
            ...
            d3Helper.drawPolygon(
              d3Helper.svgGroups.npcTilesHighlight,
              [
          ...
          this.ctxTop.drawImage(
            IMAGE_BASE[obj.img.sheet].img,
            <number>obj.img.x * IMAGE_BASE[obj.img.sheet].tile_width,
            <number>obj.img.y * IMAGE_BASE[obj.img.sheet].tile_height,
            IMAGE_BASE[obj.img.sheet].tile_width,
            IMAGE_BASE[obj.img.sheet].tile_height,
            offsetX - IMAGE_BASE[obj.img.sheet].tile_half_width_floor,
            offsetY - IMAGE_BASE[obj.img.sheet].tile_half_height_floor,
            IMAGE_BASE[obj.img.sheet].tile_width,
            IMAGE_BASE[obj.img.sheet].tile_height,
          );
          continue;
        ...
        if (typeof obj.img.file == "string") {
          ...
          this.ctxTop.drawImage(
            tempImg.img,
            random_x_offset * tempImg.tile_width,
            obj.img.y * tempImg.tile_height,
            tempImg.tile_width,
            tempImg.tile_height,
            offsetX - tempImg.tile_half_width_floor,
            offsetY - tempImg.tile_half_height_floor,
            tempImg.tile_width,
            tempImg.tile_height,
          );
          continue;
    ...
    (() => {
      var canvasHolder = <HTMLCanvasElement>(
      ...
      computed_mob_locations[map_id].forEach((center, index) => {
        ...
        d3Helper.drawText(
          d3Helper.svgGroups.mobsGroups,
          npc_base[center.b_i].name,
          offsetX,
          offsetY - 30,
        );
        ...
        var jumpToGroup = (a) => {
          ...
          _offsetY = 1400 - 14 * y;
          _offsetY += 14 * x;
          map.clickOldY = $maps.scrollTop();
          map.clickOldX = $maps.scrollLeft();
        ...
        if (typeof npc_base[center.b_i].img.hash != "undefined") {
          ...
          ctx.drawImage(
            IMAGE_BASE[obj.img.sheet].img,
            obj.img.x * IMAGE_BASE[obj.img.sheet].tile_width,
            obj.img.y * IMAGE_BASE[obj.img.sheet].tile_height,
            IMAGE_BASE[obj.img.sheet].tile_width,
            IMAGE_BASE[obj.img.sheet].tile_height,
            IMAGE_BASE[obj.img.sheet].tile_width / 4,
            IMAGE_BASE[obj.img.sheet].tile_height / 4,
            IMAGE_BASE[obj.img.sheet].tile_width,
            IMAGE_BASE[obj.img.sheet].tile_height,
          );
        }
...
function drawBody(a) {
  ...
  if ("undefined" != typeof u.sheet_file) {
    ...
    if ("undefined" == typeof p) return A;
  ...
  if ((b = BODY_PARTS.PANTS[m + e] || BODY_PARTS.PANTS[m]))
    ((u = b.img),
      ...
      b.no_boots && (D = !1));
  (b = BODY_PARTS.BOOTS[w + e] || BODY_PARTS.BOOTS[w]) &&
    D &&
    ((u = b.img),
    (p = IMAGE_BASE[u.sheet]),
    (C = IMAGE_BASE[u.sheet].tile_width * u.x),
  ...
  if ((b = BODY_PARTS.LEFT_HANDS[l]))
    ((u = deepObjCopy(b.img)),
      ...
      B.drawImage(
        IMAGE_BASE[u.sheet].img,
        C,
        E + x * IMAGE_BASE[u.sheet].tile_height,
        p.tile_width,
        p.tile_height,
        11,
        11,
  ...
  (b = BODY_PARTS.HEADS[g]) &&
    -1 == NO_HEAD_HELMETS.indexOf(v) &&
    ((u = b.img),
    (p = IMAGE_BASE[u.sheet]),
    (C = IMAGE_BASE[u.sheet].tile_width * u.x),
    (E = IMAGE_BASE[u.sheet].tile_height * u.y),
...
for (var key in IMAGE_BASE) {
  ...
  if (typeof IMAGE_BASE[key].sprite != "undefined") {
    ...
    for (var building in IMAGE_BASE[key].sprite.imgs) {
      ...
      IMAGE_BASE[key].sprite.img[building].img.src =
        IMAGE_BASE[key].sprite.spriteSheetLocation;
    }
    console.log(IMAGE_BASE[key]);
  }
  if (
    typeof key == "undefined" ||
    typeof IMAGE_BASE[key] == "string" ||
    typeof IMAGE_BASE[key].url == "undefined"
  ) {
    continue;
  }
...
$.getJSON("https://rpg.mo.ee/version.js", function (data) {
  ...
  map.mapNames.forEach((value, index) => {
    ...
    mapFile.setAttribute(
      "src",
      "https://data.mo.ee/maps/map" + index + ".js?" + cache,
    );
...
function ScrollZoom(container, max_scale, factor) {
  ...
  var size = { w: target.first().width(), h: target.first().height() };
  var pos = { x: 0, y: 0 };
  ...
  function scrolled(e) {
    ...
    // pos.x = 0;
    update();
    if (typeof scrollZoom !== "undefined" && scrollZoom.updateSlider)
      scrollZoom.updateSlider();
  }
...
$("#zoom-slider").on("input", function () {
  ...
  scrollZoom.setZoom(level);
  var scale = scrollZoom.getScale();
  $("#zoom-value").text(
    scale >= 1 ? Math.round(scale) + "x" : "1/" + Math.round(1 / scale) + "x",
  );
});
...
$(document).on("keydown", function (e) {
  ...
  if (e.key === "ArrowUp") {
    e.preventDefault();
    $maps.scrollTop($maps.scrollTop() - step);
    map.clickOldY = $maps.scrollTop();
    ...
    e.preventDefault();
    $maps.scrollLeft($maps.scrollLeft() - step);
    map.clickOldX = $maps.scrollLeft();
